...
export default {
  ...
  methods: {
    ...
    alterar() {
      ...
      if (!this.corTexto.startsWith("color:")) {
        this.db.contato.colorText = "color:" + this.corTexto; // Adiciona "color:" apenas se necessário
      } else {
        this.db.contato.colorText = this.corTexto; // Usa o valor atual
      }
      // ALTERA A BORDAR
      if (this.borderVisible_) {
        this.db.contato.borderVisible = this.borderVisible_;
        this.db.contato.border = this.border_;
        console.log(this.db.contato.border);
      } else {
        this.db.contato.borderVisible = this.borderVisible_;
        this.db.contato.border = null;
      }

      this.db.contato.iconeVisible = this.iconeVisible_; // ALTERA A VISIBILIDADE  ICONE
      this.db.contato.colorIco = this.colorIcone; // ALTERA A COR DO ICONE
      this.db.contato.icone = this.icone; // ALTERA O ICONE
      this.db.contato.textVisible = this.visivelText; // ALTERA A VISIBILIDADE
      this.db.contato.texto = this.text; // ALTERA O TEXTO
  ...
  mounted() {
    ...
    this.visivelText = this.db.contato.textVisible ?? true; // INICIALIZA O CAMPO VISIVEL TEXTO
    this.iconeVisible_ = this.db.contato.iconeVisible; // INICILIZA A VISIBILIDADE DO ICONE
    this.text = this.db.contato.texto ?? ""; // INICIALIZA O CAMPO TEXTO
    this.corTexto = this.db.contato.colorText; // INICIALIZA O COR TEXTO

    this.icone = this.db.contato.icone; // INICIALIZA O ICONE
    this.colorIcone = this.db.contato.colorIco;
    this.borderVisible_ = this.db.contato.borderVisible;
  },
};
</script>
