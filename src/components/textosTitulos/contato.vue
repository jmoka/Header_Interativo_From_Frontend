...
export default {
  ...
  methods: {
    ...
    alterar() {
      this.db.textVisibleContato = this.visivel_text; // ALTERA A VISIBILIDADE
      this.db.colorTextContato = "color:" + this.corTexto; // ALTERA A COR DO TEXTO
      this.db.textoContato = this.text; // ALTERA O TEXTO
      this.saveToLocalStorage();
  ...
  mounted() {
    ...
    this.visivel_text = this.db.textVisibleContato ?? true; // INICIALIZA O CAMPO VISIVEL TEXTO
    this.text = this.db.textoContato ?? ""; // INICIALIZA O CAMPO TEXTO
    this.corTexto = this.db.colorTextContato; // INICIALIZA O CAMPO COR TEXTO
  },
};
</script>
